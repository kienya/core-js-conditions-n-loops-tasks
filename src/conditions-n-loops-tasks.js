...
function convertToRomanNumerals(num) {
  ...
  if (num <= 10) {
    romanNumber = romanNumbersArray[num - 1];
  } else {
    const decimal = num / 10;
    const remainder = num % 10;
    for (let j = 1; j < decimal; j += 1) {
      romanNumber += romanNumbersArray[romanNumbersArray.length - 1];
    }
    romanNumber += romanNumbersArray[remainder - 1];
  }
  ...
  for (let i = 0; i < numberStr.length; i += 1) {
    switch (numberStr[i]) {
      ...
      case '-':
        ...
        break;
      default:
        string += array[numberStr[i]];
    }
    if (i + 1 !== numberStr.length) string += ' ';
  }
...
function rotateMatrix(matrix) {
  ...
  for (let i = 0; i < newMatrix.length; i += 1) {
    for (let j = 0; j < newMatrix[i].length; j += 1) {
      if (i === 0) tempArray[j] = [];
      tempArray[j][newMatrix.length - 1 - i] = newMatrix[i][j];
    }
    if (i === newMatrix.length - 1) {
      for (let t = 0; t < tempArray.length; t += 1) {
        for (let n = 0; n < tempArray.length; n += 1) {
          newMatrix[t][n] = tempArray[t][n];
        }
      }
    }
  }
